...
describe('CartItem', () => {
  ...
  describe('quantity controls', () => {
    ...
    it('removes item when decrease button is clicked at quantity 1', async () => {
      const mockRemove = vi.fn();
      const item = { ...mockCartItem, quantity: 1 };
      const { user } = render(<CartItem item={item} onRemove={mockRemove} />);
    ...
    it('decrease button is not disabled at quantity 1', () => {
      const item = { ...mockCartItem, quantity: 1 };
      render(<CartItem item={item} />);
  ...
  describe('custom handlers', () => {
    ...
    it('does not call onQuantityChange when decrease at quantity 1 triggers remove', async () => {
      const mockQuantityHandler = vi.fn();
      const mockRemoveHandler = vi.fn();
      const item = { ...mockCartItem, quantity: 1 };
      const { user } = render(
        <CartItem item={item} onQuantityChange={mockQuantityHandler} onRemove={mockRemoveHandler} />
      );

      const decreaseButton = screen.getByRole('button', { name: /decrease/i });
      await user.click(decreaseButton);

      // At quantity 1, decrease triggers remove instead of quantity change
      expect(mockQuantityHandler).not.toHaveBeenCalled();
      expect(mockRemoveHandler).toHaveBeenCalledWith(item.id);
    });
  });
  ...
  describe('interaction states', () => {
    ...
    it('decrease button is clickable at quantity 1 (triggers remove)', () => {
      const item = { ...mockCartItem, quantity: 1 };
      render(<CartItem item={item} />);

      const decreaseButton = screen.getByRole('button', { name: /decrease/i });
      // At quantity 1, button is still clickable but triggers remove instead
      expect(decreaseButton).toHaveClass('cursor-pointer');
    });
  });
